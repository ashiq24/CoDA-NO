...
def get_ssl_models_codaNo(
    ...
    """
    Get the SSL models for CoDA-NO.

    Args:
        params: The parameters for the models.
        module (Type[CodANO]): The module type for the models.
        block (Type[TNOBlock]): The block type for the models.
        convolution (Type[Union[SpectralConvKernel2d, SpectralConvolutionKernel3D]]): The convolution type for the models.
        logger (Optional[logging.Logger], optional): The logger for logging messages. Defaults to None.
        verbose (bool, optional): Whether to enable verbose mode. Defaults to True.

    Returns:
        Tuple: A tuple containing the encoder, decoder, contrastive, and predictor models.
    """
...
def get_ssl_models_codano_gino(params):
    """
    Get the SSL models for CoDA-NO GINO.

    Args:
        params (object): Parameters for the models.
    ...
    output_mesh = (
        torch.transpose(torch.stack([x.flatten(), y.flatten()]), dim0=0, dim1=1,)
        .type(torch.float)
        .cuda()
    )
...
class SSLWrapper(nn.Module):
    ...
    def _encode_variable(self, x: torch.Tensor, encoder):
        ...
        static_channels = [
            n + len(encoding_channels) for n in range(self.n_static_channels)
        ]

        y = torch.zeros(
            (batch_size, token_size, *domain_size), device=x.device, dtype=x.dtype,
        )
        y[:, 0, ...] = x
    ...
    def encode_variables(self, x: torch.Tensor, encoders):
        ...
        """
        xs = [
            self._encode_variable(x[:, i], encoder)
    ...
    def forward(
        ...
        Args:
            ...
            static_random_tensor: Static random tensor (optional).
            equations (Optional[torch.Tensor]): Tensor containing wrapped integers corresponding to Equation enum (optional).
            out_grid_displacement: Output grid displacement (optional).
            in_grid_displacement: Input grid displacement (optional).

        Returns:
            torch.Tensor: Output tensor.

        Raises:
            ValueError: If the equation(s) are not defined for a variable encoding.
            ValueError: If equations in a batch are not of the same kind.
            ValueError: If the stage is not one of the expected values.
        """
        if self.grid_type == "uniform":
        ...
        if self.stage == StageEnum.RECONSTRUCTIVE:
            return self.forward_reconstructive(
                x_embedded, in_grid_displacement, out_grid_displacement,
            )
        ...
        raise ValueError(
            f"Expected stage to be one of {list(StageEnum)};\n" f"Got {self.stage=}"
        )

    def do_mask(self, x):
        """
        Applies masking to the input tensor `x` if masking is enabled.

        Args:
            x (torch.Tensor): The input tensor to be masked.

        Returns:
            torch.Tensor: The masked input tensor if masking is enabled, otherwise returns the input tensor as is.
        """
        if not self.masking:
            return x
        ...
        with torch.no_grad():
            x_masked, masks = batched_masker(
                x, self.augmenter_masker, batched_channels=self.next_channels,
            )
    ...
    def forward_reconstructive(
        self, x, in_grid_displacement=None, out_grid_displacement=None,
    ):
    ...
    def forward_predictive(
        self, x: torch.Tensor, in_grid_displacement=None, out_grid_displacement=None,
    ):
        """
        Performs forward predictive inference using the given input tensor `x`.

        Args:
            x (torch.Tensor): The input tensor for forward prediction.
            in_grid_displacement (torch.Tensor, optional): The input grid displacement tensor. Defaults to None.
            out_grid_displacement (torch.Tensor, optional): The output grid displacement tensor. Defaults to None.

        Returns:
            Tuple[torch.Tensor, None, None, None]: A tuple containing the output tensor, and three None values.
