...
class CondnoGino(nn.Module):
    def __init__(
        ...
        if self.lifting:
            ...
            # a varibale + it's varibale encoding + the static channel together
            # constitute a token
            ...
            hidden_token_codim = (
                self.in_token_codim + var_enco_channels + self.static_channels_num
            )
        ...
        if enable_cls_token:
            ...
            self.cls_token = VariableEncoding2d(
                hidden_token_codim,
                (var_enco_mode_x, var_enco_mode_y),
                basis=var_enco_basis,
            )

    def get_output_scaling_factor(self, initial_scale, scalings_per_layer):
        """
        Calculates the output scaling factor based on the initial scale and scalings per layer.

        Args:
            initial_scale (float or list): The initial scaling factor.
            scalings_per_layer (list): A list of scaling factors per layer.

        Returns:
            float or list: The output scaling factor.
    ...
    def get_device(self):
        """
        Get the device of the coefficients_r tensor.

        Returns:
            torch.device: The device of the coefficients_r tensor.
        """
    ...
    def forward(self, inp):
        ...
        Args:
            inp (Tensor): Input tensor of shape (batch_size, n_points, in_dims/Channels).

        Returns:
            Tensor: Output tensor of shape (batch_size, n_points, out_dims/Channels).
        """
        inp = inp[0, :, :]
        inp = inp[None, ...]
        if self.re_grid_input:
        ...
        if self.enable_cls_token:
            ...
            x = torch.cat(
                [cls_token[None, :, :, :].repeat(x.shape[0], 1, 1, 1), x], dim=1
            )
