...
class Projection(nn.Module):
    def __init__(
        ...
        Args:
            ...
            hidden_channels (int, optional): Number of hidden channels. Defaults to None.
            n_dim (int, optional): Number of dimensions for convolution. Defaults to 2.
            non_linearity (function, optional): Non-linearity function to apply. Defaults to F.gelu.
            permutation_invariant (bool, optional): Whether to apply permutation invariance. Defaults to False.
        """
        super().__init__()
...
class ProjectionT(Projection):
    ...
    This class represents a time-aware projection MLP layer. It inherits from the `Projection` class.
    ...
    Attributes:
        norm (nn.InstanceNorm3d): Instance normalization layer.
    """
...
class VariableEncodingArgs(NamedTuple):
    """
    Represents the arguments for variable encoding.

    Args:
        basis (Literal["sht", "fft"]): The basis used for encoding.
        n_channels (int): Number of extra encoding channels per variable.
        modes_x (int): Number of modes in the x-direction.
        modes_y (int): Number of modes in the y-direction.
        modes_t (Optional[int]): Number of modes in the t-direction (optional).
...
class CodANO(nn.Module):
    ...
    def __init__(
        ...
        if self.lifting:
            ...
            self.lifting = self._mk_lifting_operator(
                n_lifted_channels, hidden_token_codimension, lifting_token_codimension,
            )
    ...
    def _initialize_variable_encoding_channels(
        self, n_variables, variable_encoding_args: VariableEncodingArgs,
    ):
        """
    ...
    def _mk_lifting_operator(
        self, n_lifted_channels, hidden_token_codimension, lifting_token_codimension,
    ):
        return Projection(
    ...
    def forward(self, x: torch.Tensor):
        ...
        if self.enable_cls_token:
            ...
            x = torch.cat([cls_token.repeat(*repeat_shape), x,], dim=1,)
...
class CoDANOTemporal(CodANO):
    ...
    def _mk_variable_encoder(self, ve_args):
        ...
        return FourierVariableEncoding3D(channel_size=ve_args.n_channels, modes=modes,)
