...
class CodANO(nn.Module):
    ...
    var_enco_channels=1 : number of channels for each variable encodings
    var_enco_mode_x=50 : number of x modes for each variable encodings
    ...
    def __init__(
        ...
        integral_operator_bottom=partial(SpectralConvKernel2d,
            ...
            if self.static_channels_num != 0:
                for i in self.variable_channels:
                    for j in range(i + 1, i + self.static_channels_num + 1, 1):
                        self.static_channels.append(j)

            self.encoding_channels = list(
                set(range((self.static_channels_num + self.var_enco_channels + 1) * var_num))
        ...
        n_lifted_channels = self.in_token_codim + var_enco_channels + self.static_channels_num
        if self.lifting:
    ...
    def forward(self, inp):
        ...
        if self.var_encoding:
            ...
            # Unsqueeze `var_encoding` in 0th dimension and repeat it
            # until it matches the dimension of batch_sizes:
