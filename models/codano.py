...
class CodANO(nn.Module):
    ...
    def __init__(self,
        ...
        if var_encoding:
            ...
            self.var_encoding_funtions = VaribaleEncoding2d(
                var_num * self.var_enco_channels,
                mode_x=var_enco_mode_x,
                mode_y=var_enco_mode_y,
                basis=var_enco_basis)
            self.variable_channels = [i *
                                      (self.static_channels_num +
                                       self.var_enco_channels +
                                       1) for i in range(var_num)]
        ...
        if self.lifting:
            ...
            self.lifting = Projection(
                in_channels=self.in_token_codim +
                var_enco_channels +
                self.static_channels_num,
    ...
    def forward(self, inp):
        ...
        if self.var_encoding:
            x = torch.zeros((inp.shape[0], len(self.variable_channels) + len(self.encoding_channels) + len(
                self.static_cahnnels), inp.shape[2], inp.shape[3]), device=inp.device, dtype=inp.dtype)
            ...
            if self.static_channels_num != 0:
                x[:, self.static_cahnnels, :, :] = self.static_features[:, :, :, :].repeat(
                    x.shape[0], len(self.static_cahnnels) // self.static_features.shape[1], 1, 1)
        else:
            x = inp
