...
class ConvolutionTestCase(unittest.TestCase):
    def assertConvolutionIsLike(
        self, expected: ConvolutionParameters, fno_block: FNOBlocks,
    ):
...
class AbstractTNOBlockTest(ConvolutionTestCase):
    """
    This class represents a test case for the AbstractTNOBlock class.

    Attributes:
        tno_block (Optional[TNOBlock]): The TNOBlock instance to be tested.
        token_codimension (Optional[int]): The token codimension value.
        n_modes (Optional[tuple[int]]): The number of modes.
        n_dim (Optional[int]): The number of dimensions.
        norm (Optional[Any]): The normalization function.

    Methods:
        setUp(): Set up the test case.
        _check_initialization(n_heads: int, kqv_non_linear: bool): Check the initialization of the TNOBlock instance.
    """
    ...
    def _check_initialization(self, n_heads: int, kqv_non_linear: bool):
        """
        Check the initialization of the TNOBlock instance.

        Args:
            n_heads (int): The number of attention heads.
            kqv_non_linear (bool): Whether to use non-linear activation for key, query, and value convolutions.
        """
...
class TNOBlock2DTest(AbstractTNOBlockTest):
    ...
    def test_compute_attention(self):
        ...
        self.assertEqual(
            (batch_size * 4, self.token_codimension, length, length), tuple(ya.shape),
        )
    ...
    def test_forward_propagation(self):
        ...
        self.assertEqual(
            (batch_size, self.token_codimension * 4, length, length), tuple(ya.shape),
        )
    ...
    def test_backwards_propagation(self):
        ...
        displacement_in = torch.tensor(
            np.sin(xx * np.sqrt(2.0)) + np.sin(yy * np.sqrt(3.0)), device=DEVICE,
        )
        # These tensors need to be expanded because of the sparse ``numpy``
        # representation from ``meshgrid``
        displacement_code1 = torch.tensor(np.sin(xx), device=DEVICE).expand(
            displacement_in.shape
        )
        displacement_code2 = torch.tensor(np.sin(yy), device=DEVICE).expand(
            displacement_in.shape
        )
        ...
        # These tensors need to be expanded because of the sparse ``numpy``
        # representation from ``meshgrid``
        velocity_code1 = torch.tensor(np.sin(xx + yy), device=DEVICE).expand(
            velocity_in.shape
        )
        velocity_code2 = torch.tensor(np.sin(yy - xx), device=DEVICE).expand(
            velocity_in.shape
        )
        ...
        for _ in range(10):
            ...
            loss = loss_fn(
                ...
                velocity_out.float().view(1, -1), velocity_pred.float().view(1, -1)
            )
            loss.backward()
...
class TNOBlock3DTest(AbstractTNOBlockTest):
    ...
    def test_compute_attention(self):
        ...
        ya = self.tno_block.compute_attention(xa, batch_size)
        self.assertEqual(
            (batch_size * 4, self.token_codimension, length // 4, length, length,),
            tuple(ya.shape),
        )
    ...
    def test_forward_propagation(self):
        ...
        ya = self.tno_block(xa, batch_size)  # forward pass
        self.assertEqual(
            (batch_size, self.token_codimension * 4, length // 4, length, length,),
            tuple(ya.shape),
        )
    ...
    def test_backwards_propagation(self):
        ...
        displacement_code3 = torch.tensor(np.sin(tt), device=DEVICE).expand(
            displacement.shape
        )
        ...
        velocity_code2 = torch.tensor(np.sin(yy + tt), device=DEVICE).expand(
            velocity.shape
        )
        velocity_code3 = torch.tensor(np.sin(tt + xx), device=DEVICE).expand(
            velocity.shape
        )
        ...
        for _ in range(10):
            ...
            loss = loss_fn(
                ...
                velocity[t1:t2].float().view(1, -1), velocity_pred.float().view(1, -1)
            )
            loss.backward()
            losses.append(loss.item())
            optimizer.step()
