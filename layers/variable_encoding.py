...
class VaribaleEncoding2d(nn.Module):
    ...
    def forward(self, x):
        ...
        if self.basis == 'sht' and (self.transform.nlat != size_x or self.transform.nlon != size_y) :
                self.transform = th.InverseRealSHT(size_x, size_y, lmax=self.mode_x, mmax= self.mode_y, \
                                                grid = "legendre-gauss", norm= 'backward').to(self.coefficients_i.device, dtype=self.coefficients_i.dtype)
        if self.basis == 'sht':
            return self.transform(self.coefficients_r+ 1.0j*self.coefficients_i)
        else:
            return self.transform(self.coefficients_r+ 1.0j*self.coefficients_i, s=(size_x,size_y)).real
