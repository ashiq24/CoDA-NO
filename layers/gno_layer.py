...
class GnoPremEq(nn.Module):
    def __init__(
        ...
        if self.var_encoding:
            ...
            self.encoding_channels = list(
                set([i for i in range((var_encoding_channels + 1) * var_num)])
        ...
        self.projection = MLPLinear(
            [self.var_encoding_channels + self.in_dim, projection_hidden_dim, out_dim],
        ...
        if self.end_projection:
            self.end_projector = MLPLinear(
                [self.out_dim, projection_hidden_dim, self.end_projection_outdim],
                non_linearity=F.gelu,
            )

    def update_grid(self, input_grid=None, output_grid=None):
        """
        Update the grid with new input and output grids.

        Args:
            input_grid (torch.Tensor, optional): The input grid. If not provided, the current input grid will be used.
            output_grid (torch.Tensor, optional): The output grid. If not provided, the current output grid will be used.
        """
        ...
        if self.fixed_neighbour:
            ...
            self.neighbour = NS(
                input_grid.clone().cpu(), input_grid.clone().cpu(), radius=self.radius
            )
    ...
    def _intergral_transform(self, x):
        """
        Applies the integral transform to the input tensor.

        Args:
            x: The input tensor.

        Returns:
            The transformed tensor after applying the integral transform.
        """
        for i in range(self.n_layers):
            if i == self.n_layers - 1:
                x = self.it[i](
                    self.input_grid, self.neighbour_last, self.output_grid, x
                )
                if self.end_projection:
                    x = self.end_projector(x)
            else:
                x = self.it[i](self.input_grid, self.neighbour, self.input_grid, x) + x
        return x

    def forward(self, inp):
        """
        Forward pass of the GNO layer.
        ...
        if self.var_encoding:
            x = torch.zeros(
                (
                    inp.shape[0],
                    inp.shape[1],
                    len(self.variable_channels) + len(self.encoding_channels),
                ),
                device=inp.device,
                dtype=inp.dtype,
            )
            ...
            x[:, :, self.encoding_channels] = var_encoding[None, :, :].repeat(
                x.shape[0], 1, 1
            )
        else:
            x = inp
        ...
        x = rearrange(
            x, "b n (v c) -> (b n) v c", c=self.in_dim + self.var_encoding_channels
        )
        x = self.projection(x)
...
class GNO(nn.Module):
    def __init__(
        ...
        self.radius = radius
        # project to higher dim
        self.projection = MLPLinear([self.in_dim, projection_hidden_dim, out_dim])
    ...
    def update_grid(self, input_grid=None, output_grid=None):
        """
        Updates the input and output grids of the GNO layer.

        Args:
            input_grid (torch.Tensor, optional): The input grid to be updated. If not provided, the current input grid of the layer will be used.
            output_grid (torch.Tensor, optional): The output grid to be updated. If not provided, the current output grid of the layer will be used.
        """
        if input_grid is None:
            input_grid = self.input_grid
        if output_grid is None:
        ...
        if self.fixed_neighbour:
            NS = FixedNeighborSearch(use_open3d=False)
            self.neighbour = NS(
                input_grid.clone(), output_grid.clone(), n_neigbor=self.n_neigbor
            )
        else:
            NS = NeighborSearch(use_open3d=False)
            self.neighbour = NS(
                input_grid.clone(), output_grid.clone(), radius=self.radius
            )
