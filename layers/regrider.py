...
class Regird(nn.Module):
    def __init__(self, input_grid, output_grid, sht_nlat = 128, sht_nlon = 256,output_scaling_factor=None):
        super().__init__()
        self.input_transform = th.RealSHT(sht_nlat, sht_nlon, grid=input_grid, norm='backward').float()
        self.input_grid = input_grid
        self.output_grid = output_grid
        self.output_transform = th.InverseRealSHT(sht_nlat, sht_nlon, grid=output_grid, norm='backward').float()
