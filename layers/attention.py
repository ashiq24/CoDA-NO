...
class TnoBlock2d(nn.Module):
    def __init__(self, n_modes,
        ...
        # attention per channel not per varibales
        ...
        if self.n_head is not None:
            # recaluculating the value of head_codim
        ...
        self.attention_normalizer = nn.InstanceNorm2d(
            self.token_codim, affine=True)

        # we have an option to make the last operator (MLP in regular Transformer block) permutation eq.
        # i.e., applying the operator per vairable or applying the operator on
        # all the channel (like regular FNO)
        if permutation_eq:
            ...
            self.mixer_out_normalizer = nn.InstanceNorm2d(
                self.mixer_token_codim, affine=True)
        else:
            self.mixer = FNOBlocks(
