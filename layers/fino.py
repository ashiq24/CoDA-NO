...
class SpectralConvKernel2d(SpectralConv):
    ...
    def __init__(self, in_channels, out_channels, n_modes, incremental_n_modes=None, bias=True,
                 ...
                 rank=0.5, factorization='dense', implementation='reconstructed', fno_block_precision='full', 
        ...
        super().__init__(in_channels, out_channels, n_modes, incremental_n_modes, bias=bias,\
                 ...
                 joint_factorization=joint_factorization, decomposition_kwargs=decomposition_kwargs,\
                 init_std=init_std, fft_norm=fft_norm)
    ...
    def reset_parameter(self):
        ...
        scaling_factor = ((1/self.in_channels)**0.5)/(self.half_n_modes[0]*self.half_n_modes[1])
    ...
    def forward(self, x, indices=0, output_shape = None):
        ...
        if self.frequency_mixer:
            ...
            x[:,:, :self.half_n_modes[0], :self.half_n_modes[1]] = self.mode_mixer(x[:,:, :self.half_n_modes[0],\
                                                                                     :self.half_n_modes[1]].clone(), W1)
            x[:,:, -self.half_n_modes[0]:, :self.half_n_modes[1]] = self.mode_mixer(x[:,:, -self.half_n_modes[0]:,\
                                                                                      :self.half_n_modes[1]].clone(), W2)
