...
class SpectralConvKernel2DWrapper(torch.nn.Module):
    ...
    def forward(
        ...
        return self.convolution.forward(x, indices=indices, output_shape=output_shape)
...
class SpectralConvKernel2DTest(unittest.TestCase):
    ...
    def test_forward_transform_fft(self):
        convolution = SpectralConvKernel2d(
            in_channels=4, out_channels=4, n_modes=(8, 8), transform_type="fft",
        )
    ...
    def test_inverse_transform_fft(self):
        ...
        y = convolution.inverse_transform(
            x, target_height=physical_resolution, target_width=physical_resolution,
        )
        self.assertEqual(tuple(y.shape), (1, physical_resolution, physical_resolution))
    ...
    def test_forward_propagation(self):
        convolution = SpectralConvKernel2d(
            in_channels=4, out_channels=8, n_modes=(16, 16), transform_type="fft",
        )
    ...
    def test_backwards_propagation(self):
        ...
        optimizer = torch.optim.Adam(
            convolution_w.parameters(), lr=0.04, weight_decay=0.004,
        )
        ...
        for _ in range(10):
            ...
            [[position_pred, velocity_pred]] = convolution_w.forward(
                torch.concatenate(
                    [displacement_in.unsqueeze(0), velocity_in.unsqueeze(0),]
                ).unsqueeze(0)
            )

            loss = loss_fn(
                displacement_out.float().view(1, -1), position_pred.float().view(1, -1)
            ) + loss_fn(
...
class SpectralConvKernel3DTest(unittest.TestCase):
    ...
    def test_forward_transform_fft(self):
        convolution = SpectralConvolutionKernel3D(
            in_channels=4, out_channels=4, n_modes=(8, 8, 8), transform_type="fft",
        )
    ...
    def test_forward_propagation(self):
        convolution = SpectralConvolutionKernel3D(
            in_channels=4, out_channels=8, n_modes=(16, 16, 16), transform_type="fft",
        )
    ...
    def test_backwards_propagation(self):
        ...
        loss_fn = torch.nn.MSELoss()
        optimizer = torch.optim.Adam(
            convolution_w.parameters(), lr=1.0e-4, weight_decay=1.0e-5,
        )
        ...
        for _ in range(10):
            ...
            loss = loss_fn(
                ...
                velocity[100:200].float().view(1, -1), velocity_pred.float().view(1, -1)
            )
            loss.backward()
            losses.append(loss.item())
            optimizer.step()
