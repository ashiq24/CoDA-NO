...
class ResizeDataset(Dataset):
    """
    A dataset class that resizes the input and target tensors to a specified resolution.

    Args:
        parent_dataset (Dataset): The parent dataset.
        resolution (tuple): The desired resolution for resizing.
    ...
    def __getitem__(self, global_idx):
        """
        Retrieves the item at the specified global index.

        Args:
            global_idx (int): The global index of the item.

        Returns:
            tuple: A tuple containing the resized input and target tensors.
        """
        x, y = self.parent_dataset[global_idx]
        xp = F.interpolate(
    ...
    def __call__(self, size):
        ...
        for i in augmented_channels[drop_len:]:
            ...
            while n_drop_pix > 0:
                ...
                mask_height = min(random.randint(self.min_block, mx_blk_height), h - y0)
                mask_width = min(random.randint(self.min_block, mx_blk_width), w - x0)
...
class MaskerUniformIndependent:
    ...
    def __call__(self, size, channels: Optional[Tuple[int]] = None):
        ...
        if channels is None:
            ...
            augmented_channels = np.random.choice(
                channels, min(n_augmented_channels, len(channels)), replace=False,
            )
        ...
        for i in augmented_channels:
            for t in range(time_duration):
                ...
                while n_drop_pix > 0:
                    ...
                    # Pick lengths of the mask along each dimension:
                    mask_height = min(
                        random.randint(self.min_block, max_block_height), height - y0
                    )
                    mask_width = min(
                        random.randint(self.min_block, max_block_width), width - x0
                    )
...
class MaskerNonuniformMesh(object):
    def __init__(
        ...
        self.in_nbr = torch.where(dists <= radius, 1.0, 0.0).long()
    ...
    def __call__(self, size):
        ...
        if p < 0.5:
            drop_len = int(
                self.channel_drop_rate * math.ceil(C * self.channel_aug_rate)
            )
            mask[:, augmented_channels[:drop_len]] = 0.0
            return None, mask
        else:
...
def get_meshes(input_mesh_location, grid_size):
    """
    Retrieves the input and output meshes based on the given input mesh location and grid size.

    Args:
        input_mesh_location (str): The location of the input mesh.
        grid_size (tuple): The size of the grid (size_x, size_y).

    Returns:
        tuple: A tuple containing the input mesh and the output mesh.
    """
    ...
    output_mesh = (
        torch.transpose(torch.stack([x.flatten(), y.flatten()]), 0, 1)
        .type(torch.float)
        .cuda()
    )
